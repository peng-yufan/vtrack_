...
# '''-------------------------------'''
# from lib.models.videotrack.VideoTrack_dy import build_videonet as build_videonet_test
# '''-------------------------------'''
class VideoTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        #     network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        # except:
        #     network = build_videonet_test(params.cfg, training=False)
        #     network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        ...
        self.aver_num = 1
        try:
            self.inter_long_min = self.cfg.TEST.SAMPLE_EPOCH.LONG_MIN
            self.inter_short_min = self.cfg.TEST.SAMPLE_EPOCH.SHORT_MIN
            self.inter_max = self.cfg.TEST.SAMPLE_EPOCH.LONG_MAX
            self.inter_long_max = self.inter_max
            self.inter_short_max = self.cfg.TEST.SAMPLE_EPOCH.SHORT_MIN
        except:
            self.inter_long_min = 5
            self.inter_short_min = 3
            self.inter_max = 30
            self.inter_long_max = self.inter_max
            self.inter_short_max = 8

        self.inter_long = self.inter_long_min
        # self.inter_long = 30
        self.inter_short = self.inter_short_min
        # self.inter_long = 10
        self.sample_id_long = [(self.num_search - self.aver_num)*(self.inter_max-self.inter_long)
                                +self.inter_long * i for i in range(self.num_search - self.aver_num)]
        self.sample_id_short = [(self.num_search - self.aver_num)*(self.inter_max-self.inter_short)
                                +self.inter_short * i for i in range(self.num_search - self.aver_num)]
        ...
        # self.inter = 1
        # self.reset_epoch = 800
        # self.reset_epoch = int(self.num_search * self.inter_long_max * 7)
        ...
        if hasattr(self.cfg.TEST.UPDATE_INTERVALS, DATASET_NAME):
            ...
            # self.online_sizes = [2]
        try:
            self.using_sigmoid = self.cfg.TEST.UNLINEAR.SIGMOID.USING
            self.sigmoid_factor = self.cfg.TEST.UNLINEAR.SIGMOID.FACTOR
        except:
            self.using_sigmoid = False
        self.update_interval = self.update_intervals[0]
        self.online_size = self.online_sizes[0]
        # self.online_size = 1
        if hasattr(params.cfg.TEST.RESET_EPOCH, DATASET_NAME):
            self.reset_epoch = params.cfg.TEST.RESET_EPOCH[DATASET_NAME][0]
        else:
            self.reset_epoch = 800
    ...
    #     plt.imshow(z_patch_arr)
    #     plt.imsave(str(k)+'.jpg', z_patch_arr)
    #     return
    # #
        ...
        self.search_features = [out_dict['search_feature']] * (self.num_search-self.aver_num)*self.inter_max
        self.template_features = [out_dict['template_feature']] * (self.num_search-self.aver_num)*self.inter_max
        self.pred_boxes = [box_cxcywh_to_xyxy(out_dict['pred_boxes'].view(-1, 4))] * (self.num_search-self.aver_num)*self.inter_max
    ...
    def reset_search(self, image, state):
        ...
        self.search_features = [out_dict['search_feature']] * (self.num_search - self.aver_num) * self.inter_max
    # def set_inter(self):
    #     frame_id = self.frame_id % self.reset_epoch-1
    #     if frame_id % self.inter_long_max == 1 and self.inter_long<=self.inter_long_max:
    #         self.inter_long += 1
    #     if frame_id % self.inter_short_max == 1 and self.inter_short<=self.inter_short_max:
    #         self.inter_short += 1
    #     self.sample_id_long = [(self.num_search - self.aver_num)*(self.inter_max-self.inter_long)
    #                             +self.inter_long * i for i in range(self.num_search - self.aver_num)]
    #     self.sample_id_short = [(self.num_search - self.aver_num)*(self.inter_max-self.inter_short)
    #                             +self.inter_short * i for i in range(self.num_search - self.aver_num)]
        # print(self.search_features.__len__())
    def set_inter(self):
        frame_id = self.frame_id % self.reset_epoch-(self.num_search - self.aver_num)
        if frame_id % (self.num_search - self.aver_num) == 1 and self.inter_long<=self.inter_long_max:
            self.inter_long += 1
        if frame_id % (self.num_search - self.aver_num) == 1 and self.inter_short<=self.inter_short_max:
            self.inter_short += 1
        self.sample_id_long = [(self.num_search - self.aver_num)*(self.inter_max-self.inter_long)
                                +self.inter_long * i for i in range(self.num_search - self.aver_num)]
        self.sample_id_short = [(self.num_search - self.aver_num)*(self.inter_max-self.inter_short)
    ...
    def track(self, image, info: dict = None):
        ...
        #     k = random.randint(0,99)
        #     plt.imshow(x_patch_arr)
        #     plt.imsave(str(k)+'_'+str(self.frame_id)+'.jpg', x_patch_arr)
        #     if self.frame_id==50:
        #         return
        search = self.preprocessor.process(x_patch_arr)
        with torch.no_grad():
        ...
        " long"
        with torch.no_grad():
            searches = [self.search_features[i] for i in self.sample_id_long]
        ...
        pred_boxes_robust = pred_boxes
        ' short'
        with torch.no_grad():
            searches = [self.search_features[i] for i in self.sample_id_short]
            searches = torch.stack(searches+self.aver_num * [out['search_feature']], dim=1)
            # searches = torch.stack(self.num_search * [out['search_feature']], dim=1)
            # searches = torch.stack(self.aver_num * [out['search_feature']], dim=1)
            out_bb = self.network.head_bbox(searches.permute([1, 0, 2, 3, 4]))
            # out_bb = self.network.head_bbox(searches.permute([1, 0, 2, 3, 4])
            #                                 .view(-1, searches.shape[2], searches.shape[3],  searches.shape[4]))
            # out_bb = out_bb.unsqueeze(1).permute([1, 2, 0])
        pred_boxes = box_xyxy_to_cxcywh(torch.mean(out_bb[:, :, -self.aver_num:], dim=-1).view(-1, 4))
        pred_boxes_precise = pred_boxes
        ...
        if  self.using_sigmoid:
            iou = (self.sigmoid_factor*(iou-0.5)).sigmoid()
        ...
        if self.frame_id % self.reset_epoch == 0:
        ...
        # print(self.frame_id, '  ', pred_score)
        # self.state = state
        if self.debug:
