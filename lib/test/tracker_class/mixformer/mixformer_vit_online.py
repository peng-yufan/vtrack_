...
class MixFormerOnline(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        template = self.preprocessor.no_process(z_patch_arr)
        ...
        self.frame_id = 0
        if not self.save_all_boxes:
            out ={'resize_factor': resize_factor}
            return out
        elif self.save_all_boxes:
    ...
    def track(self, image, info: dict = None):
        ...
        search = self.preprocessor.no_process(x_patch_arr)
        with torch.no_grad():
            if self.online_size==1:
        ...
        if pred_score > 0.5 and pred_score > self.max_pred_score:
            ...
            self.online_max_template = self.preprocessor.no_process(z_patch_arr)
        ...
        if self.save_all_boxes:
            ...
            return {"target_bbox": self.state,
                    ...
                    'search_feature': out_dict['search_feature'],
                    'template_feature': out_dict['template_feature'],
                    'resize_factor': resize_factor}
        else:
            return {"target_bbox": self.state,
                    'search_feature': out_dict['search_feature'],
