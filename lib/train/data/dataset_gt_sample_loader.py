...
from lib.train.data.util import dataset_gt_video, opencv_loader, processing_sample_gt, LTRLoader, pil_loader
from lib.train.data.util.loader import LTRLoader_prefetch
...
def build_seq_dataloaders(cfg, settings):
	...
	data_processing_train = processing_sample_gt.SequenceProcessing(search_area_factor=settings.search_area_factor,
	...
	data_processing_val = processing_sample_gt.SequenceProcessing(search_area_factor=settings.search_area_factor,
	                                                    output_sz=settings.output_sz,
	                                                    center_jitter_factor=settings.center_jitter_factor,
	                                                    scale_jitter_factor=settings.scale_jitter_factor,
	...
	dataset_train = dataset_gt_video.VideoDataset(
	...
	loader_train = LTRLoader_prefetch('train', dataset_train, training=True, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=shuffle,
	                         num_workers=cfg.TRAIN.NUM_WORKER, drop_last=True, stack_dim=1, sampler=train_sampler,
							 )# pin_memory=True)

	# Validation samplers and loaders
	dataset_val = dataset_gt_video.VideoDataset(
	...
	loader_val = LTRLoader_prefetch('val', dataset_val, training=False, batch_size=cfg.TRAIN.BATCH_SIZE,
