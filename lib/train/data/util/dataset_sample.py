...
class VideoDataset(torch.utils.data.Dataset):
	def __init__(self, datasets, p_datasets, samples_per_epoch,
	             ...
	             train_cls=False, pos_prob=0.5, sample_inter=None):
	...
	def _sample_sequence_ids(self, visible, num_ids=1, min_id=None, max_id=None):
		...
		if self.sample_inter == None:
			max_inter = min(int(len(visible)/num_ids), 20)
	...
	def getitem(self):
		...
		while not valid:
			...
			if is_video_dataset:
				...
				# 											  allow_invisible=False)
				search_frame_ids = self._sample_sequence_ids(visible, num_ids=self.num_search_frames)
			...
			try:
				...
				masks_template = anno_template['mask'] if 'mask' in anno_template else [torch.zeros((H, W))] * self.num_template_frames
				...
				masks_search = anno_search['mask'] if 'mask' in anno_search else [torch.zeros((H, W))] * self.num_search_frames


				data = TensorDict({'template_images': frames_template,
				                   'template_bboxes': anno_template['bbox'],
				                   'template_masks': masks_template,
				                   # 'sequence_images': frames_sequence,
				                   # 'sequence_bboxes': anno_sequence['bbox'],
				                   # 'sequence_masks': masks_sequence,
				                   'search_images': frames_search,
				                   'search_bboxes': anno_search['bbox'],
				                   'search_masks': masks_search,
	...
	# 				template_ids += random_choice(list(range(1, self.num_template_frames + self.num_sequence_frames+self.num_search_frames)),
	# 											  k=self.num_template_frames - 1, is_repeat=False)
	#
	# 			sequence_ids = random_choice(list(range(1, self.num_template_frames + self.num_sequence_frames+self.num_search_frames)),
	# 										 k=self.num_sequence_frames, is_repeat=False)
	#
	# 			search_ids = random_choice(list(range(1, self.num_template_frames + self.num_sequence_frames+self.num_search_frames)),
	# 										 k=self.num_search_frames, is_repeat=False)
	#
	# 			# invisible search image
	# 			if random.random() < self.pos_prob:
	# 				label = torch.ones(1, )
	# 				search_images = [frames[i] for i in search_ids]
	# 				search_bboxes = [anno['bbox'][i] for i in search_ids]
	# 				search_masks = [masks[i] for i in search_ids]
	# 			else:
	# 				label = torch.zeros(1, )
	# 				search_images = [frames[i] for i in search_ids]
	# 				search_bboxes = [anno['bbox'][i] for i in search_ids]
	# 				search_masks = [masks[i] for i in search_ids]
	# 				images = []
	# 				for i in range(search_images.__len__()):
	# 				# for image, bbox in zip(search_images, search_bboxes):
	# 					image = search_images[i]
	# 					bbox = search_bboxes[i]
	# 					images.append(mask_image(image, bbox))
	...
	# 				# 	n_search_images, search_anno, meta_obj_test = self.get_one_search()
	# 				# else:
	# 				# 	n_search_images, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids,
	# 				# 	                                                               seq_info_dict)
	# 				#
	# 				# H, W, _ = n_search_images[0].shape
	# 				# n_search_bboxes = [self.get_center_box(H, W)]
	# 				# n_search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H, W))]
	# 				#
	# 				# search_images = [frames[i] for i in search_ids[0:self.num_search_frames-1]]
	# 				# search_bboxes = [anno['bbox'][i] for i in search_ids[0:self.num_search_frames-1]]
	# 				# search_masks = [masks[i] for i in search_ids[0:self.num_search_frames-1]]
	# 				#
	# 				# search_images.append(n_search_images[0])
	# 				# search_masks.append(n_search_masks[0])
	# 				# search_bboxes.append(n_search_bboxes[0])
	#
	#
