...
class SequenceDataset_VAL(torch.utils.data.Dataset):
	def __init__(self, start, path, processing=None, sequence_len=None):
	...
	def __len__(self):
		return self.video_number - self.start
	...
	def getitem(self):
		...
		while not valid:
			# Select a dataset
			data_id = random.randint(self.start, self.video_number)
