...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'gt_bbox', 'mask', 'att', 'att_box']
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                ...
                if var_name in ['coords', 'bbox', 'gt_bbox']:
    ...
    def transform_gt_bbox(self, bbox, image_shape, *rand_params):
        """Assumes [x, y, w, h]"""
        # Check if not overloaded
        if self.transform_coords.__code__ == TransformBase.transform_coords.__code__:
            return bbox
