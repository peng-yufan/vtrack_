...
def get_optimizer(net, cfg):
	...
	if train_cls or train_score:
		...
		param_dicts = [
			{"params": [p for n, p in net.named_parameters() if "score" not in n and p.requires_grad]},
			{
...
def get_optimizer_mynet(net, cfg):
	# print('only train mynet! ')
	# param_dicts = [
	# 	{"params": [p for n, p in net.named_parameters() if "backforward_net" in n or "pos_embed_t" in n or "pos_embed_s" in n]}
	# ]
	# param_dicts = [
	# 	{"params": [p for n, p in net.named_parameters() if "time" in n or 'score' in n],
	# 	 "lr": cfg.TRAIN.LR,
	# 	 "weight_decay": cfg.TRAIN.WEIGHT_DECAY}
	# ]
	# param_dicts_pretrain = [
	# 	{"params": [p for n, p in net.named_parameters() if "time" not in n and 'score' not in n and 'backbone' not in n],
	# 	 "lr": cfg.TRAIN.LR/10,
	# 	 "weight_decay": cfg.TRAIN.WEIGHT_DECAY}
	# ]
	# param_dicts = param_dicts + param_dicts_pretrain

	param_dicts = [
		{"params": [p for n, p in net.named_parameters() if "backbone" not in n and 'score' not in n],
		 "lr": cfg.TRAIN.LR,
		 "weight_decay": cfg.TRAIN.WEIGHT_DECAY}
	]
	# param_dicts.append(
	# 	{"params": [p for n, p in net.named_parameters() if "backbone" not in n and 'score' in n],
	# 	 "lr": cfg.TRAIN.LR/10,
    #      # "lr": cfg.TRAIN.LR ,
	# 	 "weight_decay": cfg.TRAIN.WEIGHT_DECAY})
	# print()
	# param_dicts = [
	# 	{"params": [p for n, p in net.module.head_bbox.named_parameters() if 'time' in n],
	# 	 "lr": cfg.TRAIN.LR,
	# 	 "weight_decay": cfg.TRAIN.WEIGHT_DECAY}
	# ]
	# param_dicts.append(
	# 	{"params": [p for n, p in net.module.head_bbox.named_parameters() if 'time' not in n],
	# 	 "lr": cfg.TRAIN.LR / 10,
	# 	 # "lr": cfg.TRAIN.LR ,
	# 	 "weight_decay": cfg.TRAIN.WEIGHT_DECAY})
	# param_dicts.append(
	# 	{"params": [p for n, p in net.module.head_score.named_parameters() ],
	# 	 "lr": cfg.TRAIN.LR/10 ,
	# 	 # "lr": cfg.TRAIN.LR ,
	# 	 "weight_decay": cfg.TRAIN.WEIGHT_DECAY})
	...
	# param_dicts = [
	# 	{"params": [p for n, p in net.named_parameters() if 'backbone' not in n]}
	# ]
	# param_dicts = [
	...
	# opt_pretrain_args = dict(lr=lr/100, weight_decay=weight_decay)
	opt_lower = cfg.TRAIN.OPTIMIZER.lower()
	if opt_lower == 'sgd' or opt_lower == 'nesterov':
		opt_args.pop('eps', None)
		optimizer = torch.optim.SGD(param_dicts, momentum=cfg.TRAIN.MOMENTUN, nesterov=True, **opt_args)
