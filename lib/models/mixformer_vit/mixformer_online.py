...
class Attention(nn.Module):
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        ...
        template_len = q.shape[2] - s_h * s_w
        q_mt, q_s = torch.split(q, [template_len, s_h * s_w], dim=2)
        k_mt, k_s = torch.split(k, [template_len, s_h * s_w], dim=2)
        v_mt, v_s = torch.split(v, [template_len, s_h * s_w], dim=2)

        # asymmetric mixed attention
        attn = (q_mt @ k_mt.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x_mt = (attn @ v_mt).transpose(1, 2).reshape(B, template_len, C)
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward(self, x_t, x_ot, x_s):
        ...
        # x = torch.cat([x_m, x_t, x_ot, x_s], dim=1)
        x = self.pos_drop(x)
        # i = 0
        ...
        # x_m, x_t, x_ot, x_s = torch.split(x, [H_t*W_t, H_t*W_t,L * H_t*W_t, H_s*W_s], dim=1)
        x_t, x_ot, x_s = torch.split(x, [H_t * W_t, H_t * W_t, H_s * W_s], dim=1)

        x_t_2d = x_t.transpose(1, 2).reshape(B, C, H_t, W_t)
        x_ot_2d = x_ot.transpose(1, 2).reshape(B, C, H_t, -1)
    ...
    def forward_test(self, x):
        ...
        x_s_2d = rearrange(x, 'b (h w) c -> b c h w', h=H_s, w=H_s)
        # from matplotlib import pyplot as plot
        # x_s_2 = torch.sum(torch.abs(x_s_2d[0]), dim=0)
        # x_s_2 = x_s_2.detach().to('cpu')
    ...
    def set_online(self, x_t, x_ot):
        ...
        x_t = x_t + self.pos_embed_t
        x_ot = x_ot + self.pos_embed_t
        x_ot = x_ot.reshape(b, -1, x_ot.size(-1))  # [1, num_ot * H_t * W_t, C]
...
class MixFormerOnlineScore(nn.Module):
    ...
    def forward(self, template, online_template, search, run_score_head=True, gt_bboxes=None, as_backbone=False):
        # search: (b, c, h, w)
        if template.dim() == 5:
        ...
        # if self.training:
        #     template = torch.cat([memory['m'].view(template.shape[0], template.shape[2], template.shape[3],
        #                                            # template.shape[1]).permute([0, 3, 1, 2]), ], dim=-1)
        #                                            template.shape[1]).permute([0, 3, 1, 2]), template], dim=-1)
        out, outputs_coord_new = self.forward_head(search, template, run_score_head, gt_bboxes)
        out.update({'search_feature':search,
    ...
    def set_online_mynet(self, x):
        self.backbone.set_online_mynet(x)
...
def build_mixformer_vit_online_score(cfg, settings=None, train=True) -> MixFormerOnlineScore:
    ...
    try:
        ...
        missing_keys, unexpected_keys = model.load_state_dict(ckpt['net'], strict=True)
    #     if is_main_process():
    #         print("missing keys:", missing_keys)
    #         print("unexpected keys:", unexpected_keys)
