...
class VisionTransformer(nn.Module):
	...
	def forward(self, template, online_template, memory=None, search=None, online_size=2):
		"""
       ...
       """
		...
		if template.shape[1] == 3:
			...
			if memory == None:
				memory = torch.zeros([template.shape[0], 0, self.patch_size**2]).cuda()
				online_size = 1
			...
			search = self.patch_embed(search)
			search = search + self.search_pos_embed
			t_H, t_W = int(template.shape[1]**0.5), int(template.shape[1]**0.5)
			s_H, s_W = int(search.shape[1] ** 0.5), int(search.shape[1] ** 0.5)


		# apply Transformer blocks
		x = torch.cat([memory, template, online_template, search], dim=1)
		if self.use_cls_token:
		...
		# batch = 0
		# xab = torch.abs(x)
		# m = (torch.mean(xab[batch, 1: 1 + 64], dim=-1)).view(8, 8).detach().to('cpu')
		# i = (torch.mean(xab[batch, 1 + 64: 1 + 2 * 64], dim=-1)).view(8, 8).detach().to('cpu')
		# d = (torch.mean(xab[batch, 1 + 2 * 64: 1 + 3 * 64], dim=-1)).view(8, 8).detach().to('cpu')
		...
		# xab = torch.abs(x)
		# batch = 0
		# s = (torch.mean(xab[batch, 1 + 3 * 64:], dim=-1)).view(20, 20).detach().to('cpu')
		# plt.figure(figsize=(20, 20))
		# plt.imshow(s)

		if self.use_cls_token:
			cls_tokens, memory, template, online_template, search = torch.split(x, [1, t_H * t_W, t_H * t_W, t_H * t_W, s_H * s_W], dim=1)
		else:
			memory, template, online_template, search = torch.split(x, [t_H * t_W, t_H * t_W, t_H * t_W, s_H * s_W], dim=1)
		m_dict = {
			'm0': memory0.detach(),
			'x': search.detach(),
			'z': torch.cat([template.detach(), online_template.detach()], dim=-2),
			'm': memory.detach()
		}
		...
		if self.use_cls_token:
			return template, online_template, search, m_dict, cls_tokens,
		return template, online_template, search, m_dict, None
