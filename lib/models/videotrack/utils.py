...
def combine_tokens(template_tokens, search_tokens, add_template_tokens = [], mode='direct', return_res=False):
    # [B, HW, C]
    len_t = template_tokens.shape[1]
    len_s = search_tokens.shape[1]

    if mode == 'direct':
        if add_template_tokens !=[]:
            merged_feature = torch.cat((add_template_tokens,template_tokens, search_tokens), dim=1)
        else:
