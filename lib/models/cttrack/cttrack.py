...
class CTTrack(nn.Module):
	...
	def forward(self, template, online_template, memory=None, search=None, run_score_head=True):
		# input image: (b, c, h, w)

		template, online_template, search, m_dict, cls_tokens = self.backbone(template, online_template, memory, search)
		# Forward the corner head
		out_dict, outputs_coord = self.forward_head(search, cls_tokens, run_score_head)
	...
	def forward_head(self, search, cls_tokens, run_score_head=True):
		...
		out_dict.update({'search_feature': search})
	...
	# img = torch.mean(torch.abs(search[2]), dim=0)
	# plt.figure(figsize=(20, 20))
	# plt.imshow(img.to('cpu').detach())
...
def build_cttrack(cfg):
	...
	# checkpath = './CTTrack-B.pth.tar'
	# model.load_state_dict((torch.load(checkpath, map_location=torch.device('cpu')))['net'], strict=False)
	# checkpath = './CTTrack_train_ep0020.pth.tar'
	# model.load_state_dict((torch.load(checkpath, map_location=torch.device('cpu')))['net'], strict=False)
	return model
