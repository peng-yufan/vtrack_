...
class CTTrack_train(nn.Module):
	...
	def forward(self, template, online_template, search, target_in_search, gt_bboxes, memory,
	            t2t=False, t2s=False, s2t=False, s2s=False):
		# template, online_template: (b, c, h_t, w_t)
		# search: (b, c, h_s, w_s)
		# gt_box: (b,4)

		template_feat, online_template_feat, search_feat, m_dict,cls_tokens = self.backbone(template, online_template, memory,
		                                                                             search)
		outputs_coord_new = self.forward_box_head(search_feat, cls_tokens)
...
def build_cttract_train(cfg):
	...
	# cross_decoder = build_maskdecoder(cfg)
