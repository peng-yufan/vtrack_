...
class Mul_Pyramid_Corner_Predictor(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False, search_num=1):
        ...
        """ mix on time channel"""
        self.time_conv_tl2 = conv(search_num, search_num)
        self.time_conv_tl3 = conv(search_num, search_num)
        self.time_conv_tl4 = conv(search_num, search_num)

        self.time_conv_br2 = conv(search_num, search_num)
        self.time_conv_br3 = conv(search_num, search_num)
        self.time_conv_br4 = conv(search_num, search_num)
    ...
    def forward(self, x, return_dist=False, softmax=True):
        """ Forward pass with input x.,  N, B, C, Hs, Ws """
        score_map_tl, score_map_br = self.get_score_map(x)
        if return_dist:
            coorx_tl, coory_tl, prob_vec_tl = self.soft_argmax(score_map_tl, return_dist=True, softmax=softmax)
            coorx_br, coory_br, prob_vec_br = self.soft_argmax(score_map_br, return_dist=True, softmax=softmax)
            return torch.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=2) / self.img_sz, prob_vec_tl, prob_vec_br
        else:
            coorx_tl, coory_tl = self.soft_argmax(score_map_tl)
            coorx_br, coory_br = self.soft_argmax(score_map_br)
            return torch.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=2) / self.img_sz

    def get_score_map(self, x):
        # N, B, C, Hs, Ws
        N, B, C, H, W = x.shape
        x = x.view(-1, C, H, W)  # NB, C, H, W
        x_init = x
        # top-left branch
        x_tl1 = self.conv1_tl(x)
        x_tl2 = self.conv2_tl(x_tl1)
        x_tl2 = x_tl2.view(N, B, -1, 1 * H, 1 * W).permute([1, 2, 0, 3, 4]).view(-1, N, 1*H, 1*W) #bc, n, h, w
        x_tl2 = self.time_conv_tl2(x_tl2).view(B, -1, N, 1 * H, 1 * W).permute([2, 0, 1, 3, 4]).reshape(B*N, -1, 1*H, 1*W)
        ...
        x_tl3 = x_tl3.view(N, B, -1, 2 * H, 2 * W).permute([1, 2, 0, 3, 4]).view(-1, N, 2*H, 2*W)
        x_tl3 = self.time_conv_tl3(x_tl3).view(B, -1, N, 2 * H, 2 * W).permute([2, 0, 1, 3, 4]).reshape(B*N, -1, 2*H, 2*W)
        ...
        x_tl4 = x_tl4.view(N, B, -1, 4 * H, 4 * W).permute([1, 2, 0, 3, 4]).view(-1, N, 4*H, 4*W)
        x_tl4 = self.time_conv_tl4(x_tl4).view(B, -1, N, 4 * H, 4 * W).permute([2, 0, 1, 3, 4]).reshape(B*N, -1, 4*H, 4*W)
        ...
        score_map_tl = score_map_tl.view(N, B, 1, 4 * H, 4 * W)
        # bottom-right branch
        x_br1 = self.conv1_br(x)
        x_br2 = self.conv2_br(x_br1)
        x_br2 = x_br2.view(N, B, -1, 1 * H, 1 * W).permute([1, 2, 0, 3, 4]).view(-1, N, 1 * H, 1 * W)  # bc, n, h, w
        x_br2 = self.time_conv_br2(x_br2).view(B, -1, N, 1 * H, 1 * W).permute([2, 0, 1, 3, 4]).reshape(B * N, -1,
                                                                                                        1 * H, 1 * W)
        ...
        x_br3 = x_br3.view(N, B, -1, 2 * H, 2 * W).permute([1, 2, 0, 3, 4]).view(-1, N, 2 * H, 2 * W)
        x_br3 = self.time_conv_br3(x_br3).view(B, -1, N, 2 * H, 2 * W).permute([2, 0, 1, 3, 4]).reshape(B * N, -1,
                                                                                                        2 * H, 2 * W)
        ...
        x_br4 = x_br4.view(N, B, -1, 4 * H, 4 * W).permute([1, 2, 0, 3, 4]).view(-1, N, 4 * H, 4 * W)
        x_br4 = self.time_conv_tl4(x_br4).view(B, -1, N, 4 * H, 4 * W).permute([2, 0, 1, 3, 4]).reshape(B * N, -1,
                                                                                                        4 * H, 4 * W)
        ...
        score_map_br = score_map_br.view(N, B, 1, 4 * H, 4 * W)
        return score_map_tl, score_map_br

    def soft_argmax(self, score_map, return_dist=False, softmax=True):
        """ get soft-argmax coordinate for a given heatmap """
        N, B, _, H, W = score_map.shape
        score_vec = score_map.view((N, -1, self.feat_sz * self.feat_sz))  # (N*batch, feat_sz * feat_sz)
        prob_vec = nn.functional.softmax(score_vec, dim=-1)
        exp_x = torch.sum((self.coord_x * prob_vec), dim=-1)
        exp_y = torch.sum((self.coord_y * prob_vec), dim=-1)
        if return_dist:
