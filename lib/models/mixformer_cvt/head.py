...
class Mul_Pyramid_Corner_Predictor(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False, num_search=1):
        ...
        '''top-left corner'''
        self.conv1_tl = conv(inplanes * num_search, channel * num_search, freeze_bn=freeze_bn)
        self.conv2_tl = conv(channel * num_search, channel * num_search // 2, freeze_bn=freeze_bn)
        self.conv3_tl = conv(channel * num_search // 2, channel * num_search // 4, freeze_bn=freeze_bn)
        self.conv4_tl = conv(channel * num_search // 4, channel * num_search // 8, freeze_bn=freeze_bn)
        self.conv5_tl = nn.Conv2d(channel * num_search // 8, num_search, kernel_size=1)

        self.adjust1_tl = conv(inplanes * num_search, channel // 2 * num_search, freeze_bn=freeze_bn)
        self.adjust2_tl = conv(inplanes * num_search, channel // 4 * num_search, freeze_bn=freeze_bn)

        self.adjust3_tl = nn.Sequential(conv(channel * num_search // 2, channel * num_search // 4, freeze_bn=freeze_bn),
                                        conv(channel * num_search // 4, channel * num_search // 8, freeze_bn=freeze_bn),
                                        conv(channel * num_search // 8,  1 * num_search, freeze_bn=freeze_bn))
        self.adjust4_tl = nn.Sequential(conv(channel * num_search // 4, channel * num_search // 8, freeze_bn=freeze_bn),
                                        conv(channel * num_search // 8,  1 * num_search, freeze_bn=freeze_bn))

        '''bottom-right corner'''
        self.conv1_br = conv(inplanes * num_search, channel * num_search, freeze_bn=freeze_bn)
        self.conv2_br = conv(channel * num_search, channel * num_search // 2, freeze_bn=freeze_bn)
        self.conv3_br = conv(channel * num_search // 2, channel * num_search // 4, freeze_bn=freeze_bn)
        self.conv4_br = conv(channel * num_search // 4, channel * num_search // 8, freeze_bn=freeze_bn)
        self.conv5_br = nn.Conv2d(channel * num_search // 8, num_search, kernel_size=1)

        self.adjust1_br = conv(inplanes * num_search, channel // 2 * num_search, freeze_bn=freeze_bn)
        self.adjust2_br = conv(inplanes * num_search, channel // 4 * num_search, freeze_bn=freeze_bn)

        self.adjust3_br = nn.Sequential(conv(channel * num_search // 2, channel * num_search // 4, freeze_bn=freeze_bn),
                                        conv(channel * num_search // 4, channel * num_search // 8, freeze_bn=freeze_bn),
                                        conv(channel * num_search // 8, 1 * num_search, freeze_bn=freeze_bn))
        self.adjust4_br = nn.Sequential(conv(channel * num_search // 4, channel * num_search // 8, freeze_bn=freeze_bn),
                                        conv(channel * num_search // 8, 1 * num_search, freeze_bn=freeze_bn))
    ...
    def soft_argmax(self, score_map, return_dist=False, softmax=True):
        ...
        score_vec = score_map.view((-1, self.num_search,self.feat_sz * self.feat_sz))  # (batch, feat_sz * feat_sz)
